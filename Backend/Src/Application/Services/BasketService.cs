...
public class BasketService(DataContext db, IMapper mapper, ILogger<BasketService> logger, IPaymentGateway payments) : IBasketService
{
    ...
    public async Task<Result<bool>> DeleteBasketAsync(int id)
    {
        await _db.Baskets.AsNoTracking().Where(x => x.Id == id).ExecuteDeleteAsync();
        var saved = await _db.SaveChangesAsync() > 0;
        if (!saved) return Result<bool>.Fail("Unable to delete basket.", ResultTypeEnum.Invalid);
    ...
    public async Task<Result<BasketDto>> AddCouponAsync(BasketCouponDto dto)
    {
        ...
        basket.AttachPaymentIntentInfo(piModel.IntentId, piModel.ClientSecret ?? "");
